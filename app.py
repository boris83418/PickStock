...
# 算出月營收
def get_monthly_revenue(stock_number):
    # 構建查詢股票營收的 URL
...
def index():
    if request.method == 'POST':
        ...
        # 獲取股票月營收數據
        financial_data = get_monthly_revenue(stock_number)
        # 獲取股票淨利
        ...
        return render_template('index.html', stock_data=df, k_line_plot=k_line_plot,
                               financial_data=financial_data, profit_data=profit_data)
    
    # 用戶未提交前都為none
    return render_template('index.html', stock_data=None, k_line_plot=None,
                           financial_data=None,profit_data=None)
